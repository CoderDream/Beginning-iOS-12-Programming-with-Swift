...
extension AppDelegate: UNUserNotificationCenterDelegate {
    func userNotificationCenter(_ center: UNUserNotificationCenter,
        ...
        if response.actionIdentifier == "foodpin.makeReservation52" {
            print("Make reservation... Step 1 ... ")
            print("phone: \(String(describing: response.notification.request.content.userInfo["phone"]))")
            if let phone = response.notification.request.content.userInfo["phone"] {
                let telURL = "tel://\(phone)"
                print("Make reservation...  Step 2 ")
                if let url = URL(string: telURL) {
                    print("Make reservation...  Step 3 ")
                    if UIApplication.shared.canOpenURL(url) {
                        print("Make reservation...  Step 4 ")
